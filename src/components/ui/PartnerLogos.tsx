...
// Use premium brand colors instead of Tailwind's gray/blue
  ...
  return (
    <section className="w-full bg-pearl-white/95 py-10 border-t border-b border-imperial-gold-400/15 overflow-hidden shadow-sm">
      <div className="text-center mb-6 px-2">
        <h2 className="text-xl md:text-2xl font-extrabold text-ink-black tracking-tight" style={{ letterSpacing: "-0.5px" }}>
        ...
        <div className="flex animate-premium-scroll space-x-10 md:space-x-14 w-max px-6 py-3">
          {[...partners, ...partners].map((partner, index) => (
            <div key={`${partner.name}-${index}`} className="relative group flex flex-col items-center min-w-[90px] md:min-w-[108px]">
              <div className="rounded-xl p-2 md:p-3 bg-white/80 border border-imperial-gold-400/10 shadow-md hover:shadow-lg transition-all flex items-center justify-center">
                <img
              ...
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 px-3 py-2 rounded bg-ink-black text-pearl-white text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10 border border-silk-crimson-400/20 shadow-lg font-medium">
                {partner.tooltip}
              </div>
      ...
      <p className="text-center text-sm md:text-base text-ink-black/60 mt-8 max-w-2xl mx-auto font-medium">
